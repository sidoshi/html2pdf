...
use std::{collections::HashMap, num::NonZeroUsize, sync::Mutex, time::{SystemTime, UNIX_EPOCH}};
...
impl TokenValidator {
    ...
    pub async fn validate_token(&self, token: &str) -> Result<TokenValidationResult> {
        ...
        if claims.token_requested_from.is_some() || claims.user_id.is_some() || claims.customer_id.is_some() {
        ...
        // Validate with JWKS
        self.validate_jwks_token(token, issuer, &header, &claims).await
    }
    ...
    async fn validate_ship_token(&self, token: &str, _claims: &Claims) -> Result<TokenValidationResult> {
        let ship_key = self.config.ship_symmetric_key
            .as_ref()
    ...
    async fn validate_jwks_token(
        ...
        let jwks_url = self.config.jwks_issuers
            .iter()
    ...
    async fn get_decoding_key_from_jwks(&self, jwks_url: &str, kid: &str) -> Result<DecodingKey> {
        ...
        // Fetch JWKS
        let jwks: Value = self.http_client
            .get(jwks_url)
            .send()
            .await?
            .json()
        ...
        let keys = jwks["keys"].as_array().ok_or(AuthError::InvalidTokenFormat)?;
